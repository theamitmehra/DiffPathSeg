...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; padding: 20px 24px; }
...
<body>
  <main class="panel">
    <section class="head">
      <div>
        ...
        <p class="sub">Run jobs, validate data, train/eval, and fetch artifacts</p>
      </div>
  ...
  <script>
    ...
    function renderArtifacts(payload) {
      ...
      if (!files.length) { artifactsEl.textContent = "No artifacts found for this job yet."; return; }
    ...
    async function loadArtifacts(jobId) {
      ...
      const { res, payload } = await fetchJson(`/v1/jobs/${jobId}/artifacts`);
      if (!res.ok) { setStatus(`Artifacts fetch failed (${res.status}).`, "err"); artifactsEl.textContent = JSON.stringify(payload, null, 2); return; }
    ...
    async function validateJob(jobId) {
      if (!jobId) { setStatus("Enter a job_id first.", "err"); return; }
      const { res, payload } = await fetchJson(`/v1/jobs/${jobId}/validation`);
      if (!res.ok) { setStatus(`Validation failed (${res.status}).`, "err"); outputEl.textContent = JSON.stringify(payload, null, 2); return; }
      outputEl.textContent = JSON.stringify(payload, null, 2);
      const invalid = payload.summary ? payload.summary.invalid_samples : 0;
      setStatus(`Validation complete for ${jobId}. invalid=${invalid}`, invalid > 0 ? "err" : "ok");
    }

    async function trainEvalJob(jobId) {
      if (!jobId) { setStatus("Enter a job_id first.", "err"); return; }
      const { res, payload } = await fetchJson(`/v1/jobs/${jobId}/train_eval`);
      if (!res.ok) { setStatus(`Train/eval failed (${res.status}).`, "err"); outputEl.textContent = JSON.stringify(payload, null, 2); return; }
      outputEl.textContent = JSON.stringify(payload, null, 2);
    ...
    async function fetchJob(jobId, silent = false) {
      if (!jobId) { if (!silent) setStatus("Enter a job_id first.", "err"); return; }
      const { res, payload } = await fetchJson(`/v1/jobs/${jobId}`);
      if (!res.ok) { setStatus(`Job lookup failed (${res.status}).`, "err"); outputEl.textContent = JSON.stringify(payload, null, 2); return; }
      outputEl.textContent = JSON.stringify(payload, null, 2);
      setStatus(`Job ${payload.job_id} is ${payload.status}.`, payload.status === "failed" ? "err" : "ok");
      if (payload.status === "completed") await loadArtifacts(payload.job_id);
      return payload;
    }
    ...
    async function createJob() {
      ...
      if (rawOverride) {
        try { override = JSON.parse(rawOverride); }
        catch (_) { setStatus("Override JSON is invalid.", "err"); runBtn.disabled = false; return; }
      }
      ...
      try {
        const res = await fetch("/v1/jobs", { method: "POST", headers: getHeaders(), body: JSON.stringify(payload) });
        const text = await res.text();
        let data = {};
        try { data = JSON.parse(text); } catch { data = { raw: text }; }
        if (!res.ok) { setStatus(`Job creation failed (${res.status}).`, "err"); outputEl.textContent = JSON.stringify(data, null, 2); return; }
    ...
    checkBtn.addEventListener("click", async () => { const jobId = document.getElementById("jobId").value.trim(); try { await fetchJob(jobId); } catch (e) { setStatus(`Request failed: ${e.message}`, "err"); } });
...
def _run_job(job_id: str, cfg: AppConfig) -> None:
    ...
    try:
        ...
        output_root = Path(str(metrics.get("output_dir", cfg.output_dir / job_id)))

        try:
            persistence = persist_job_artifacts(job_id=job_id, output_root=output_root)
            metrics["artifact_store"] = persistence.get("mode", "local")
            metrics["artifact_manifest"] = persistence.get("files", [])
...
def _local_artifact_files(job_id: str, root: Path) -> List[Dict[str, Any]]:
    ...
    for kind in ["synthetic", "curated"]:
        ...
        for pattern in ["*.png", "*.pgm"]:
            for p in sorted(directory.glob(pattern)):
                media = "image/png" if p.suffix.lower() == ".png" else "image/x-portable-graymap"
                files.append(
                    {
                        "kind": kind,
                        "name": p.name,
...
def _manifest_files(job: JobRecord) -> List[Dict[str, Any]]:
    if not job.metrics:
        return []
    manifest = job.metrics.get("artifact_manifest")
    if not isinstance(manifest, list):
        return []

    refreshed = refresh_remote_urls([dict(x) for x in manifest if isinstance(x, dict)])
    files: List[Dict[str, Any]] = []
    for item in refreshed:
        out = dict(item)
        if out.get("storage") == "s3" and out.get("remote_url"):
            out["url"] = out["remote_url"]
        files.append(out)
    return files
